...
- **소식 · 공지** (Figma `sub-05-01`, `SUB_소식공지_*_DESKTOP`, node 99:5666):
  ...
  - **상담신청** (`/news/consult`, Figma `SUB_소식공지_상담문의` / node `100:723`): 실페이지 `ConsultPage`(no local tabs) — placeholder 아님. **2단 split**: 좌 eyebrow(`Request for consultation`, mint `#58bdc2`) + 2줄 타이틀 / 우 리드 문구 + 카드(`bg #f7f7fb`). 카드 = 무료 법률 상담(전화/메일 아이콘 + 텍스트, `icon-call`/`icon-mail`) + 흰색 divider + SNS 상담문의 **다크 CTA 버튼**(`bg #111`, 흰 글리프 `icon-naver.svg`/`icon-kakao.svg`). ≤1024 세로 스택. Data `src/data/consult.ts`(SNS `href`는 실제 예약 링크 교체 예정 — 현재 `#`). Sharp-rect; brand mint(`#58bdc2`), not global teal.
