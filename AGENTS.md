...
- Claude recent UI stack already on `main` (see `WORKLOG.md` 2026-07-29):
  ...
  - Home Notice / Press: mobile **card carousels** via `src/hooks/useCardTrackNav.ts` + `.card-track-nav` (hero-arrow chrome, no glass box)
...
- **Quick rail (`QuickNav`, Figma `AI-quick` 120:974 / panel 120:1025 / compact 120:6350) — tablet+mobile only.** Mounted once in `SiteLayout`; `display:none` above 1024 (do not render it on desktop). Fixed `left:20px; bottom:20px`, `z-index:880` (under the GNB's 900). **Interaction contract (2026-07-28) — scroll decides *presence*, the user decides *size*.** Direction-driven morphing was removed: it twitched on every small reading correction, overrode taps, and had no matching mental model. Modes in `QuickNav.tsx`: `hidden` (initial paint and whenever `scrollY <= 120` — parked off-screen via `translate3d(calc(-100% - 20px)…)`, `aria-hidden` + `inert`), `compact` (default once on the page — 48px icon-only rail: toggle + divider + 위로), `expanded` (**only** when the user taps the toggle — 92px labelled rail). Presence is positional with hysteresis (`SHOW_AFTER` 120 / `HIDE_BEFORE` 80) so the edge can't flicker, and it never reacts to direction. Open closes on: same-toggle re-tap (icon rotates 45° so `+` reads `×`), outside `pointerdown`, Escape, choosing an item, leaving the page top, or scrolling ~0.9 viewport from where it was opened. **Keep the toggle rendered in both modes** — an expansion the user triggered must be reversible by the same control; the mode change animates via `width`/`padding`/`gap`/`max-height` transitions, so don't swap the markup per mode. Tokens: panel `#111` **sharp rect (no border-radius anywhere — brand chrome rule)** + shadow `0 12px 16px -1px rgba(35,48,59,.1)`, icon `#81818D`, label `#767676` 12/1.45/-0.3px. States: hover = `rgba(255,255,255,.1)` chip + white glyph; `:active` = white chip + teal glyph. **`무료법률상담` is the Figma `무료법률상담만하이라이트상태` item — it stays white at rest**, unlike the muted siblings. Icons are the uploaded `public/assets/{버튼명}.svg` files (`오시는길` / `카톡문의` / `진행사건문의` / `무료법률상담` / `위로` / `펼쳐보기`), painted with **`mask-image` + `background: currentColor`** — the files carry hard-coded fills (`#81818D` / `#767676` / `white`) so `<img>` could not recolour them across the four states. Reference the **ASCII copies** in `public/assets/quick/quick-*.svg`, never the Korean originals: `펼쳐보기.svg` is stored NFD-decomposed while the rest are NFC, so a composed URL 404s and the icon silently vanishes. Re-copy (`quick-location|kakao|case|consult|top|expand`) whenever an icon is re-uploaded. Write **standard `mask-*` only** — lightningcss adds the `-webkit-` alias (same HARD RULE as `backdrop-filter`). Labels must never clip: the rail is sized from the widest 6-glyph label (`--quicknav-w: 92px`), and `.quicknav__full` stays `overflow: visible` while expanded (clipping is applied only while collapsed).
