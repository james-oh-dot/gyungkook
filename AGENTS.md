...
- **경국인 · 갤러리** (법무법인경국 4th / Figma `sub-01-04`, `94:3369`):
  ...
  - 경국인 section: layered brand **pyramid** (Figma `94:3384`) — outer sunburst ring (`pyramid-ring.png`, CSS rotate loop), inner **white** disc (`#fff`, scale pulse **0.5 ↔ 1.0**), three SVG tiers with live labels (고객 / 경국인 / 경국); **hover/focus each tier → `scale(1.1)`**. Prefer the layered DOM over the old flat `gallery/pyramid.webp` composite. + serif **quote** (`--font-serif` NanumMyeongjo, teal `icon-quote.svg`) + **경국인의 약속** (10 rows: teal `#58bdc2` `w-34` number badge + English 32px + Korean desc; same label-left/content-right 2-col as 정비/공익).
