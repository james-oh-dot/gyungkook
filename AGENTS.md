...
- **Desktop-only 빠른문자상담 (`QuickConsult`)** — mounted in `SiteLayout` **above** `Footer`. Visible only `min-width: 1025px` (tablet/mobile: `display:none`). Dark `#111` section + form panel; privacy “내용 보기” opens a right drawer (`qc-privacy`). Copy: `src/data/quickConsult.ts`. Front-only submit mock for now — do not invent a backend.
