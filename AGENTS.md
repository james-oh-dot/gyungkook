...
- **Live == `origin/main`.** Latest includes Claude Code merges **#133–#138** (branch `claude/o-boinida-98drdm`). Pages deploys on `main` via `.github/workflows/deploy-pages.yml`.
- Claude recent UI stack already on `main` (see `WORKLOG.md` 2026-07-29):
  - Mobile hero: full-bleed + touch swipe + per-slide crop anchors + bottom prev/next (≤767 outline / touch-only states, no glass fill)
  - Home Notice / Press: mobile **card carousels** via `src/hooks/useCardTrackNav.ts` + `.card-track-nav` (52px outline frames on light bg — border only, no glass fill)
- Claude also added the top-of-file **백엔드 전달용 소스 추출** trigger (`handoff/backend-source` orphan branch) — keep that section; do not remove it when editing this file.
- Before new work: `git checkout main && git pull origin main`, then `cursor/<name>-129f`. Prefer fresh tips off `main`; do not stack on stale `claude/o-boinida-98drdm` unless asked.
- Dev readiness: `npm install` → `npm run dev` (`:5173`). Lint/build: `npm run lint` / `npm run build`. Frontend-only (no backend services).
...
- Compact (≤1024): **symbol mark** (`logo-mark.png`) sits left of the hamburger (+ tablet "메뉴" label); drawer head is mark-only too (no wordmark). Hamburger opens **left drawer** (accordion + scrim + Esc + focus trap). Drawer foot **상담 신청** CTA = brand teal (`--color-teal` / `#008075`). Do not use mega-menu hover on touch widths.
- **GNB background is 3-state (2026-07-23):** `over-hero` (top, transparent) → `glass` (scrolled, **liquid glass** frosted, `.gnb--glass`) → `solid` (white). **Scroll no longer turns the bar white** — only a menu-open **or a hover on the scrolled bar** does (`solid = menuOpen || (scrolled && hovered)`; `glass = scrolled && !solid`). `.gnb--glass` = `backdrop-filter: blur(20px) saturate(1.6)` + translucent white (standard property only — see HARD RULE below; `@supports not` fallback for old Firefox).
- **Auto text-contrast (`data-gnb-theme`):** an IntersectionObserver in `Gnb.tsx` watches a 1px band at the bar's bottom edge (`--gnb-bar-h`) and reads the `data-header-theme` of whatever section sits under it → sets `data-gnb-theme="dark|light"` on `<header>`. **Dark section under the bar → white type; else → dark type** (default `light`). Only **dark** regions need tagging: add `data-header-theme="dark"` to any full-bleed dark hero/section (currently `Hero` `.hero`, `SubVisual` `.sub-visual`, `LawyerHero` `.lawyer-hero`, `Footer` `.site-footer`). Untagged content falls back to dark type — safe on light bodies. Lightweight: no per-frame scroll math. CSS: `.gnb--over-hero[data-gnb-theme='light']` / `.gnb--glass[data-gnb-theme='light']` flip `--gnb-fg` / `--gnb-icon-filter` / indicator.
- Type/icon colors follow `--gnb-fg` + `--gnb-icon-filter`: white type over a dark section (`brightness(0) invert(1)` icons, light wordmark), dark type otherwise (`brightness(0)` icons, dark wordmark). `solid`/`open` white bar is always dark type.
- Brand chrome: **no border-radius** on GNB buttons / indicator / glass actions (sharp rect).
- Site search: `SearchOverlay` — **full-text** over `src/data/searchIndex.ts` (`searchDocs(query)`), not just nav labels. `SEARCH_DOCS` = one doc per page/route (게시판 글은 상세 페이지가 곧 문서 = 글마다 1개) whose `text` is built by **recursively harvesting every content string** from that page's data module (`collect()` skips asset paths/urls/ids/routes via `ASSET_RE` + `SKIP_KEYS`). New copy in a data file is indexed automatically — do not hand-list fields. Adding a new page = one line in `ROUTE_DOCS` (or the board/home lists). Results show the **per-page occurrence count** (`.search-overlay__count` teal badge) and rank **title-hit first, then count desc**. To index a new board: add `...boardDocs(NEW_BOARD, '대메뉴')`. Backdrop = **liquid glass**: `.search-overlay__dim` is a translucent white veil (`rgba(255,255,255,0.85)`) + `backdrop-filter: blur(28px) saturate(180%)` (+ `@supports not` denser-veil fallback). While open the background is **scroll-locked** (`document.body.overflow='hidden'` + scrollbar-width `padding-right` to prevent shift); long result lists scroll **inside** `.search-overlay__results` only (`overflow-y:auto` + `overscroll-behavior:contain`). Don't reintroduce a flat opacity dim or let scroll chain to the page.
- **`SearchOverlay` renders through a `createPortal(..., document.body)`.** It is `position: fixed; inset: 0`, but it lives inside the GNB, and the scrolled `.gnb--glass` state applies `backdrop-filter` — which **establishes a containing block for fixed descendants**, clipping the overlay to the header box. Desktop hid the bug because hovering the bar switches to the non-blurred `solid` state; tablet/mobile have no hover, so the overlay only covered the GNB strip (2026-07-28). Do not move it back inside the header tree.
- **Footer is a 3-column grid above 768** — `사업자정보 / 전화 / 팩스·메일·SNS`. `.site-footer__contact` is the **third child of `.site-footer__columns`**, not a block below it; it must never wrap onto its own row on desktop. ≤767 collapses to one column.
- **Desktop-only 빠른문자상담 (`QuickConsult`)** — mounted in `SiteLayout` **above** `Footer`. Visible only `min-width: 1025px` (tablet/mobile: `display:none`). Uses `.section.section--dark` pad (`--section-y` / `--page-pad`); left intro + right form (office-like). Title = Pretendard (`.section-title` size), **not** serif. Form fields stretch in one horizontal row (이름|전화|상담내용) + meta row (방식|동의|신청). Privacy “내용 보기” → right drawer (`qc-privacy`). Copy: `src/data/quickConsult.ts`. Front-only submit mock.
- **`.office__chip` are all `<button>`s.** The mint treatment (`#39deaf` / `#e3faf7` / `#00665e`) is the **hover / focus-visible / active** state — there is no permanently selected chip. Do not reintroduce `.is-active` on 'SMS 약도 전송'.
...
- **Quick rail (`QuickNav`, Figma `AI-quick` 120:974 / panel 120:1025 / compact 120:6350) — tablet+mobile only.** Mounted once in `SiteLayout`; `display:none` above 1024 (do not render it on desktop). Fixed `bottom:20px`, `z-index:880` (under the GNB's 900). **Placement:** expanded = `left:20px`; **compact + hidden = `left:0`** (flush to the edge). **Interaction contract (2026-07-28) — scroll decides *presence*, the user decides *size*.** Direction-driven morphing was removed: it twitched on every small reading correction, overrode taps, and had no matching mental model. Modes in `QuickNav.tsx`: `hidden` (initial paint and whenever `scrollY <= 120` — parked off-screen via `translate3d(calc(-100% - 8px)…)`, `aria-hidden` + `inert`), `compact` (default once on the page — 48px icon-only rail: toggle + divider + 위로), `expanded` (**only** when the user taps the toggle — 92px labelled rail). Presence is positional with hysteresis (`SHOW_AFTER` 120 / `HIDE_BEFORE` 80) so the edge can't flicker, and it never reacts to direction. Open closes on: same-toggle re-tap (icon rotates 45° so `+` reads `×`), outside `pointerdown`, Escape, choosing an item, leaving the page top, or scrolling ~0.9 viewport from where it was opened. **Keep the toggle rendered in both modes** — an expansion the user triggered must be reversible by the same control; the mode change animates via `width`/`padding`/`gap`/`max-height`/`left` transitions, so don't swap the markup per mode. Tokens: panel = **dense black liquid glass** (`rgba(10,10,12,.9)` + `backdrop-filter: blur(18px) saturate(1.35)` + light hairline border + **deep stacked drop shadow** — sharp rect, no border-radius; do not revert to washed `rgba(17,17,17,.62)` / solid `#111`). Standard `backdrop-filter` only (HARD RULE). Default icon/label = readable muted light (`#c8c8d0` / `#b4b4bc`) so they stay clear on dense glass but stay below the white CTA; divider `rgba(255,255,255,.32)`. **`무료법률상담` stays the highlight CTA** — full `#fff` + `font-weight: 600` at rest (Figma `무료법률상담만하이라이트상태`); do not mute it with the siblings or brighten siblings to near-white. Icons are the uploaded `public/assets/{버튼명}.svg` files (`오시는길` / `카톡문의` / `진행사건문의` / `무료법률상담` / `위로` / `펼쳐보기`), painted with **`mask-image` + `background: currentColor`** — the files carry hard-coded fills (`#81818D` / `#767676` / `white`) so `<img>` could not recolour them across the four states. Reference the **ASCII copies** in `public/assets/quick/quick-*.svg`, never the Korean originals: `펼쳐보기.svg` is stored NFD-decomposed while the rest are NFC, so a composed URL 404s and the icon silently vanishes. Re-copy (`quick-location|kakao|case|consult|top|expand`) whenever an icon is re-uploaded. Write **standard `mask-*` only** — lightningcss adds the `-webkit-` alias (same HARD RULE as `backdrop-filter`). Labels must never clip: the rail is sized from the widest 6-glyph label (`--quicknav-w: 92px`), and `.quicknav__full` stays `overflow: visible` while expanded (clipping is applied only while collapsed).
...
- **HeroClassic frame geometry (2026-07-27) — fluid, no px cap.** `.hero__bg-slide > .progressive-image` is anchored, not sized: `left = max(0px,(100% - 1920px)/2) + var(--hero-pad-x) + var(--hero-frame-gap)` (= where the copy starts, **+200px**, so that gutter is constant at every width), `right: 0` (always flush to the viewport edge), `width: auto`, and `height = var(--hero-swipe-top) - var(--hero-frame-swipe-gap)` so the frame stops exactly **20px above `.hero__swipe`**. Width/height therefore both track the viewport — do **not** reintroduce `width: min(78.125vw,1500px)` / `height: min(74.07vh,800px)` / `min(90vh,…)`. Tune via `--hero-frame-gap` / `--hero-frame-swipe-gap`. ≤1024 overrides back to `left: 0; width: 100%; height: 58%` (full-bleed tablet/mobile) — keep that override.
- **`--hero-swipe-top` is measured in `HeroClassic.tsx`** (ResizeObserver on the swipe + hero, plus `resize` and `fonts.ready`) and written onto the `.hero` element. It cannot be derived in CSS because `.hero__swipe` is `zoom`-scaled and its meta text reflows per breakpoint. Anchor off this measured **top**, not `100% - height` — the latter drifts by ~2px from sub-pixel rounding.
- **Copy sits on the swipe's line (>1024):** `.hero__content` uses `padding-bottom: calc(40px * var(--hero-swipe-scale))` (the swipe's own inner bottom padding) instead of reserving the swipe's full height, so `.hero__copy` lands beside `.hero__swipe` rather than stacked above it. ≤1024 restores `padding-bottom: var(--hero-swipe-reserve)` because there the swipe is an inset card over the copy — keep both.
- **No scroll parallax on the HeroClassic frame.** `useScrollReveal` still writes `--parallax-y` onto the slide image (its selector matches), but the frame must not consume it: translating the box makes the 20px swipe gap drift with scroll, and translating the inner bitmap instead exposes the frame edge because the full layer sits at exactly `scale(1)`. Motion is Ken Burns only.
- **HeroClassic Ken Burns contract (2026-07-27) — the frame never resizes, only the bitmap inside it:** the frame box above (`overflow: hidden` from ProgressiveImage.css) carries **only** the scroll-parallax `translate3d`. The zoom lives on the inner layers — `__full` = `scale(calc(1 + var(--hero-zoom) * 0.08))` (fills the frame exactly at rest, grows to 1.08 and is clipped), `__preview` keeps its `1.08` blur-fringe base (`scale(calc(1.08 + …))`). **Do not move `scale()` back onto the wrapper** — that grew the whole panel instead of the image. `--hero-zoom` is set inline per-frame by `HeroClassic.tsx` and inherits to the layers; reduced-motion sets both wrapper and `__full` to `transform: none`.
...
- **Mobile portrait hero arts (2026-07-30):** `public/assets/hero-M-01`…`05.png` → progressive pairs (`1170w`). Wired via `mobileImage` / `mobileImagePreview` on `slidesClassic`; `HeroClassic` swaps at ≤767. When `mobileImage` is active, skip `mobileFrame` crop hacks and use full-bleed (`.hero__bg-slide--mobile-art`). Regenerate with `python3 scripts/generate-progressive-images.py hero-M-0N.png`.
