...
- **변호사자문단** (법무법인경국 3rd / Figma `sub-01-03`):
  ...
  - Custom dark hero cover (not `SubVisual`): sub-01-03 bg + scrim + name/title + contact + **wide divider** + intro + portrait. **No eyebrow** above the name (matches Figma `89:2963`). Scrim matches Figma cover overlay: `linear-gradient(247deg, rgba(0,0,0,.88) 25.51%, rgba(102,102,102,.88) 78.43%)` — **darker on the right** (portrait side); do not flip back to left-heavy. Divider (`.lawyer-hero__divider`, Figma `Line 6` / `89:2975`) extends from the info column through the gap and **behind the portrait** (`width: calc(100% + gap + photoCol)`; photo paints above it). On mobile (portrait hidden) divider is `width: 100%`.
