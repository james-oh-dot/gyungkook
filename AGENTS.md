...
- Site search: `SearchOverlay` — index from `NAV_ITEMS`. Backdrop = **liquid glass**: `.search-overlay__dim` is a translucent white veil (`rgba(255,255,255,0.55)`) + `backdrop-filter: blur(28px) saturate(180%)` (with `-webkit-` + `@supports not` denser-veil fallback) — matches the existing GNB/close glass convention (the minifier ships these `-webkit-`-prefixed). While open the background is **scroll-locked** (`document.body.overflow='hidden'` + scrollbar-width `padding-right` to prevent shift); long result lists scroll **inside** `.search-overlay__results` only (`overflow-y:auto` + `overscroll-behavior:contain`). Don't reintroduce a flat opacity dim or let scroll chain to the page.
...
- **변호사자문단** (법무법인경국 3rd / Figma `sub-01-03`):
  ...
  - Custom dark hero cover (not `SubVisual`): sub-01-03 bg + scrim + **parent-menu label (`.lawyer-hero__eyebrow` = 법무법인경국, teal chip matching `.sub-visual__chip`)** + name/title + contact + **wide divider** + intro + portrait. (The eyebrow was added when the parent-label chip became global across all subpage heroes — see the HARD RULE at the top of Subpages; the original Figma `89:2963` had no eyebrow.) Scrim matches Figma cover overlay: desktop `247deg` / tablet `219deg` / mobile `204deg` — **darker toward the portrait side**. Divider extends behind the portrait — **portrait `z-index` must stay above info/divider** so the line never cuts through the head (opaque PNG pixels hide it; only transparent cutout margins show the stroke). **Tablet (`22:3251`) + Mobile (`22:3879`):** portrait stays visible — absolute **bottom-right** on a tall ~860px cover (do **not** `display:none` the photo on phones; do **not** put info above photo). Contact stacks vertically on ≤1024. Desktop keeps the 2-col grid.
...
- **소식 · 공지** (Figma `sub-05-01`, `SUB_소식공지_*_DESKTOP`, node 99:5666):
  - **One sub-visual for the whole section** — every child menu (소식공지 / 판례뉴스 / 인재영입) shows `sub-05-01`. `nav.ts` `GNB_SUB_VISUAL_NEWS = NEWS_NOTICE_PAGE.visual`; `news-notice` / `news-cases` / `news-careers` GNB children all point at it (`news-consult` 상담신청 stays a placeholder). Hero title per page is live/responsive over the inpainted `sub-05-01` (baked title painted out with cv2 — same pipeline as sub-01-02/04).
...
- Local tabs: route mode (`toTab`) for 컬럼미디어 / 언론보도 / 인재영입; scroll mode (`onTabSelect`) for 법인소개 / 정비사업 / 공익사업. Hover underline works in both.
