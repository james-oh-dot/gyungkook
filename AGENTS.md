...
- **HeroClassic frame geometry (2026-07-27) — fluid, no px cap.** `.hero__bg-slide > .progressive-image` is anchored, not sized: `left = max(0px,(100% - 1920px)/2) + var(--hero-pad-x) + var(--hero-frame-gap)` (= where the copy starts, **+200px**, so that gutter is constant at every width), `right: 0` (always flush to the viewport edge), `width: auto`, `height: min(90vh, 100%)`. Width therefore grows with the viewport — do **not** reintroduce `width: min(78.125vw, 1500px)` / `height: min(74.07vh, 800px)`. Tune the gutter via `--hero-frame-gap`. ≤1024 overrides back to `left: 0; width: 100%; height: 58%` (full-bleed tablet/mobile) — keep that override.
