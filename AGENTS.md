...
- **Mobile portrait hero arts (2026-07-30):** `public/assets/hero-M-01|02|04|05.png` → progressive pairs (`1170w`). Wired via `mobileImage` / `mobileImagePreview` on `slidesClassic`; `HeroClassic` swaps at ≤767. **No `hero-M-03`** — slide 03 keeps landscape `hero-03`. When `mobileImage` is active, skip `mobileFrame` crop hacks and use full-bleed (`.hero__bg-slide--mobile-art`). Regenerate with `python3 scripts/generate-progressive-images.py hero-M-0N.png`.
