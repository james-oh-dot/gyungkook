...
## 2026-07-22 — 검색: 전체 콘텐츠 풀텍스트 + 페이지별 일치 횟수 표기

### 요구
- 키워드를 **등록된 모든 문구**에서 검색(기존엔 메뉴 라벨만 인덱싱).
- 한 페이지에 여러 번 등장하면 **그 갯수도 표기**.

### 구현
- 신규 `src/data/searchIndex.ts` — 사이트 전역 풀텍스트 인덱스.
  - `SEARCH_DOCS`: 페이지(라우트/홈 앵커) 1개 = 문서 1개. **게시판 글은 상세
    페이지가 곧 문서**(글마다 1개).
  - 각 문서 `text`는 해당 데이터 모듈의 **모든 콘텐츠 문자열을 재귀 수집**
    (`collect()`). `ASSET_RE`(자산 경로/URL/앵커/확장자) + `SKIP_KEYS`
    (id·src·icon·visual 등)로 비콘텐츠 제외 → 데이터에 문구가 추가되면
    인덱스 자동 반영(필드 하드코딩 없음, drift 방지).
  - 정적 페이지(ROUTE_DOCS 7) + 변호사(LAWYER_DOCS, 인당 1) + 홈 섹션
    (HOME_DOCS 6) + 게시판 글(BOARD_DOCS: 언론보도/컬럼미디어/사회공헌/
    소식공지/판례뉴스/채용공고) + 나머지 nav 목적지(placeholder 등, href 중복 제거).
  - `searchDocs(query)`: 페이지별 질의어 **등장 횟수**(`countOccurrences`)를
    세고, **제목 일치 우선 → 횟수 내림차순**으로 정렬.
- `SearchOverlay.tsx`: `NAV_ITEMS` 인덱스 제거 → `searchDocs` 사용.
  결과에 teal 카운트 배지(`.search-overlay__count`) + "총 N개 페이지에서 발견"
  요약(`.search-overlay__summary`). `onSubmit`은 라우트/홈앵커 모두
  `resolveNavHref`로 안전 이동(기존 raw href 버그도 수정).
- `SearchOverlay.css`: 카운트 배지(원형 pill, tabular-nums) + 요약 라인.
  모바일은 칩을 한 줄, 제목+카운트를 다음 줄에(제목 `flex:1 1 0`으로 내부
  줄바꿈, 배지는 우측 정렬 유지).

### 검증 (Playwright)
- "재개발" → 33개 페이지(정비사업 63×, 업무사례 12×, 공대호 11× …),
  "감정평가사" → 5개(공익사업 8×/법인소개 7× …), "판례" → 20개,
  "채용" → 채용공고 4건, "홈페이지 개편"(다어절) → 홈 1건, 없는 단어 → 0건.
- 결과 클릭 시 상세 라우트로 정상 이동(예: /news/notice/notice-1).
- 데스크탑/모바일 overflow-x 0, 콘솔 에러 0. `lint`/`build` 통과.

### Key files
- `src/data/searchIndex.ts`(신규), `src/components/SearchOverlay.tsx` +
  `SearchOverlay.css`.

---

## 2026-07-22 — 정비/공익/법인소개/갤러리 탭섹션 간격 + 공익 절차 흐름도 + 보상업무 라벨

### 1. 탭으로 구분된 섹션 간격 (Figma)
스크롤-탭 페이지의 **탭 섹션 사이** 간격을 데스크탑 **400px** / 태블릿·모바일(≤1024)
**88px**, **탭 내부 중간섹션** 간격을 데스크탑 **140px** / 태블릿·모바일 **74px**로 통일.
- 정비사업 `Renewal.css`: `.renewal__contents` 140→400; ≤1024에서 88, `.renewal-section`/
  `.renewal-split__body` 74. 768/767의 기존 80·64·56 gap 오버라이드는 제거(≤1024 규칙 상속).
- 공익사업: `Renewal.css`를 재사용(`.renewal__contents`/`.renewal-section`)하므로 자동 반영.
- 법인소개 `AboutIntro.css`: 데스크탑은 이미 400/140. ≤1024 200→88·80→74, ≤767 120→88·56→74.
- 갤러리 `Gallery.css`: `.gallery__main`(경국인·갤러리 사이) clamp→400, ≤1024 88;
  `.gallery-people` clamp→140, ≤1024 74.
- 검증(Playwright computed rowGap): 4개 페이지 모두 데스크탑 400/140, ≤1024·모바일 88/74.

### 2. 공익사업 절차·유의할 점 — Figma 정확 반영 (node 99:4827)
- **불릿 = 기본 회색 도트**: `.pp-procedure .pp-list li::before`가 teal 플래그 SVG를
  4px 회색 원으로 오버라이드(rights 카드는 플래그 유지).
- **흐름도 라인**: 각 컬럼 카드 뒤 세로 라인(`.pp-procedure__steps::before`, left:26px,
  `--pp-flow`)을 불투명 흰색 카드가 덮어 **카드 사이 간격에서만** 보이게 → 흐름도 커넥터.
  재개발 컬럼 side 박스(명도소송/이주대책)로 가로 브랜치(`.pp-procedure__side::before`),
  `.pp-procedure__steps{overflow:hidden}`로 왼쪽 오버슈트 클립, 카드는 `__block{z-index:1}`.
- **모바일 세로 배치**: `.pp-procedures{grid-template-columns:1fr}`(≤767) — [공익사업]/[재개발]
  컬럼 세로 스택. 데스크탑·모바일 스크린샷으로 Figma 대조 확인, overflow-x 0.

### 3. 보상업무실적 라벨
`PUBLIC_RECORD_2.chip` `'재개발 · 재건축'` → `'보상업무'`. 실적 카드 카테고리 칩이
`보상업무`로 표기(검증: 고유 라벨 [공익사업, 보상업무]).

### Key files
- `src/pages/Renewal.css`, `AboutIntro.css`, `Gallery.css`, `PublicProject.css`,
  `src/data/publicProject.ts`.

---

## 2026-07-22 — 소식 · 공지 > 상담신청 (SUB_소식공지_상담문의, node 100:723)

placeholder였던 상담신청을 실페이지로 구현. Figma 2단 구성 정확 반영.

### 구성
- 서브비주얼 = 소식·공지 공통 **sub-05-01**(별도 에셋 없음). GNB 메뉴 전환
  썸네일도 `nav.ts` `news-consult` visual = `GNB_SUB_VISUAL_NEWS`로 반영.
- **2단 split**(gap 28px, 각 flex:1): 
  - 좌: eyebrow `Request for consultation`(mint #58bdc2) + 2줄 타이틀
    (`가장 편리한 방법으로 / 상담을 신청해 주세요`, #111 36px).
  - 우: 리드 3줄(#505050 18px) + 카드(bg #f7f7fb, pad 32, gap 40):
    무료 법률 상담(전화/메일 아이콘 + 텍스트) → 흰색 divider →
    SNS 상담문의 다크 버튼 2개(bg #111, 흰 글리프 + 흰 텍스트).
- **신규 아이콘**: `icon-naver.svg`(흰 N) / `icon-kakao.svg`(흰 말풍선) — 다크
  버튼용이라 `<img>`에서 흰색이 나오도록 명시 `fill:#fff`(currentColor 아님).
  전화/메일은 기존 `icon-call`/`icon-mail`(밝은 카드 위 기본 검정 아이콘).

### 반응형
≤1024 2단 → 세로 스택(타이틀 위/콘텐츠 아래). 세로 pad 140/160 → 80/120(≤768)
→ 56/96(≤767); ≤767 타이틀 26px, 리드 16px, 카드 pad 24/20.

### 배선
- `App.tsx`: `/news/consult` → `ConsultPage`(PlaceholderPage 대체).
- `nav.ts`: `news-consult` visual → `GNB_SUB_VISUAL_NEWS`(sub-05-01).
- `placeholderPages.ts`: 미사용 `news-consult` 엔트리 제거.

### 검증 (Playwright)
데스크탑/모바일 스크린샷 Figma 대조 일치, overflow-x 0, 콘솔 에러 0.
GNB 소식·공지 fullmenu에서 상담신청 hover 시 sub-05-01 노출, href `/news/consult`,
클릭 시 정상 이동. `lint`/`build` 통과.

### Follow-up
- 네이버/카카오 방문예약 실제 링크(`consult.ts` `CONSULT_SNS.links[].href`) 교체 필요.
...
## 2026-07-29 — 모바일 히어로 슬라이드별 프레이밍 + 이전/다음 버튼 복귀

### 슬라이드별 크롭 앵커 (≤767)
390×844 세로 풀블리드는 1600×900 원본의 가로 26%만 남긴다. 주제가 중앙에서 벗어난
슬라이드는 기본 center 앵커에서 반토막이 나므로 `mobileFrame`을 데이터에 추가
(`slidesClassic.ts`) → `.hero__bg-slide--frame-*` 클래스로 렌더.
- **05 `right`**: 파노라마 우측 1/3이 법원 건물. `object-position: right center`.
- **04 `flip-right`**: 켜진 가로등+비둘기가 원본 좌측에 있어 반전 없이 우측 정렬하면
  빈 밤하늘만 남는다. `--hero-flip-x: -1` + `object-position: 30% center`.
  퍼센트가 뒤집혀 읽히는 건 의도적 — 앵커는 원본 축에서 계산되고 그 뒤에
  `scaleX(-1)`이 뒤집으므로, 원본 좌측 30%가 반전 이미지의 70% 지점(가로등 위치)이다.
  0/10/…/100% 스윕 대지를 만들어 사용자가 직접 프레임 선택.
- 반전은 켄 번즈 transform에 `scaleX(var(--hero-flip-x, 1))`로 합성.
  `prefers-reduced-motion` 블록도 `transform: none` → `scaleX(var(--hero-flip-x,1))`로
  바꿔, 줌은 빠지되 프레이밍인 반전은 살아남게 했다.
- **함정**: 베이스 `object-position` 규칙이 `.hero__bg-slide > .progressive-image
  .progressive-image__full`(클래스 3개)라서 2개짜리 선택자는 조용히 진다.
  `--frame-* > .progressive-image ...`로 깊이를 맞춰야 적용된다.

### 이전/다음 버튼만 복귀 (≤767)
직전 커밋에서 `.hero__swipe`를 통째로 숨겼는데, 화살표만 살려 하단 중앙 배치.
- 썸네일 카드 / 게이지 트랙 / 일시정지 버튼은 `display: none`.
- `.hero__gage-btns`를 `position: static`으로 빼내 52px 정사각 글래스 버튼 2개를
  20px 간격 flex 행으로. 베이스의 border+blur 크롬을 그대로 쓴다.
- **함정**: 래퍼만 52px로 키우면 실제 클릭 대상인 `button`은 24px에 머문다.
  `button`을 `100%`로 채우고 `img`만 24px로 제한해야 터치 타깃이 52px가 된다.
- `.hero__content` 하단 패딩 56px → 96px(버튼 52 + 오프셋 24 + 여유).

### 검증
390에서 5슬라이드 순회: 04 `30% 50%` + matrix(-1.01…), 05 `100% 50%`, 01~03 기본.
버튼 실측 50×50 @ y=769, 좌우 중앙(134/206). 카피 하단 748로 21px 여유.
768/1440에서는 04가 `50% 50%` + 양수 matrix로 미적용 확인.
CDP 터치 스와이프 회귀 전부 통과. lint/tsc/build 통과.

## 2026-07-29 — 모바일 히어로 이전/다음 버튼 좌측 밀착

하단 중앙 → 좌측 정렬로 이동, 두 버튼을 붙였다.
- `.hero__swipe`에 `padding: 0 var(--hero-pad-x)` + `align-items: flex-start`로
  카피 좌측 끝(20px)과 라인을 맞춤. 실측 prev x=21, copy left=20.
- `.hero__swipe-controls { gap: 0 }` + `.hero__gage-btns--next { margin-left: -1px }`.
  gap을 0으로만 두면 1px 보더 두 장이 맞닿아 2px 이음매가 생기므로, 음수 마진으로
  겹쳐 단일 1px 심(segmented control 느낌)으로 만든다.

### 슬라이드 04/05 오브젝트 가시성 → 이미지 높이 68%로 확정
390×844 풀블리드는 1600×900 원본의 **가로 26%**만 남긴다. 05의 법원 건물은 원본
가로 약 40%를 차지해 어떤 `object-position`으로도 프레임에 들어오지 않았고(벽면만
보여 법원으로 인지되지 않음), 04는 가로등 아랫부분이 카피에 가렸다. 앵커 문제가
아니라 비율 문제라 높이를 줄이는 것 외에 방법이 없다.

세 안을 실제 화면으로 렌더해 비교:
- **68% (채택)**: 보이는 폭이 원본 가로 38%로 넓어져 04 가로등+비둘기 전체, 05
  타워+엠블럼+현관 아케이드까지 인지된다. **68%는 법원 건물이 들어가는 가장 얕은
  박스** — 다시 올릴 거면 05를 반드시 재확인할 것. 태블릿(58%)과도 일관된다.
- 풀블리드 유지: 위 문제 그대로.
- `contain` + 블러 백드롭: 오브젝트는 다 보이나 레터박스 스트립이라 히어로로 약함.
...
## 2026-07-29 — 캐러셀 버튼: 다크 글래스 프레임 제거, 화살표만 리컬러

히어로 버튼의 다크 글래스 박스(테두리+배경+블러)를 그대로 가져왔더니 사진이 아닌
흰/틴트 배경 위에서는 블러가 블러할 대상이 없어 "네모 안에 또 네모"처럼 보였다
(이중프레임). 요청대로 배경/테두리를 완전히 제거하고, 화살표 글리프는 히어로와
동일한 `icon-arrow.svg`를 그대로 유지 — 다만 이 파일이 `stroke="white"`로 하드코딩돼
있어 흰 배경에서 보이지 않으므로, `<img>` 대신 `<span>` + `mask-image` +
`background-color: currentColor`로 교체(QuickNav 아이콘과 동일 기법). 버튼
`color: var(--color-black)`, hover/focus는 teal — 다른 인터랙티브 요소와 통일.
52px 히트 영역과 `.is-flip` rotate(180deg)는 유지.

### 검증
스크린샷: 박스 없이 순수 화살표만 보임(이중프레임 해소), 진하게 리컬러되어
흰 배경에서도 또렷이 보임. 소식 3카드/활동보도 5카드 활성화·비활성화 회귀 재통과.
lint/tsc/build 통과.

### 브랜치 재시작
이 작업 중 PR #135이 머지됨 — 지침에 따라 `origin/main`에서 브랜치를 새로 따고
이 커밋만 얹었다(머지된 세 커밋은 이미 main에 포함, 중복 스택 없음).

## 2026-07-29 — 히어로 모바일 이전/다음 버튼: 라인형 + 터치 전용 상태 + 그림자 제거

`.hero__gage-btns`(≤767)의 채워진 다크 글래스(테두리+배경+블러+그림자)를 아웃라인
전용으로 교체.
- 이 버튼 행은 `bottom:24px`에 있어 모바일 베일의 `#000 72%` 지점보다 더 아래 —
  이미 완전한 검정 배경 위에 있어서 반투명 채움/블러가 더할 게 없었다. 배경/
  box-shadow/backdrop-filter를 전부 제거하고, 테두리 불투명도를 0.24 → 0.7로
  올려 라인 자체가 시인성을 전담하게 했다.
- 버튼(아이콘) 기본 opacity도 0.85 → 1로. 예전 0.85는 글래스 채움 위에서
  균형 잡힌 값이었는데, 채움이 없어진 지금은 그냥 흐릿해 보이기만 했다.
- **터치 전용 상태 변화**: 모바일 브라우저는 탭에서 가짜 `:hover`를 발생시키고
  간혹 그대로 눌어붙는다. `:hover`만 골라 기존 리프트(`opacity:1;
  transform:translateY(-1px)`)를 무력화(휴지 상태와 동일하게)하고, 실제 터치인
  `:active`에 눌림 피드백(opacity 0.55 + scale(0.92))을 넣었다. `:focus-visible`은
  건드리지 않음 — 키보드/스크린리더 포커스 표시는 포인터 유무와 무관해야 한다.
- 태블릿(≤1024, >767)은 손대지 않음 — 원래 글래스 크롬 그대로 유지.

### 검증
1px 테두리 rgba(255,255,255,.7), 배경 투명, box-shadow none, backdrop-filter none
확인. 마우스 호버 시 opacity/transform 불변(호버 무력화 확인). 마우스 다운 중
opacity/transform이 :active 목표값으로 전환 중인 것 확인. 클릭 시 슬라이드 정상
전환(02→03). 1024px에서는 원래 글래스 크롬(배경/블러/그림자/0.24 테두리) 그대로
유지 확인. lint/tsc/build 통과.

## 2026-08-05 — 홈 히어로 개편 + 협력사 무한 띠 + 메뉴명 3건

### 히어로: swipe 제거 → 엣지 화살표
`.hero__swipe` 블록 전체(썸네일 프리뷰 + 메타 + 게이지 트랙 + 일시정지)를 제거하고
`.hero__nav--prev/--next` 두 개만 남겼다. 크기 36px → **108px**(3배), 화면 좌우 끝에
`top:50%` 세로 중앙 배치. 태블릿 84 / 모바일 56으로 단계 축소 — 390px 화면에서
문자 그대로 3배(108)면 화면 폭의 1/4를 먹는다.

제거에 딸려온 정리:
- `--hero-swipe-top` 측정 로직이 사라진 요소에 매달려 있었다. `.hero__copy`를 CSS에서
  bottom 앵커(`--hero-copy-bottom`)로 바꾸고, JS는 `--hero-maincopy-top` 충돌 방지
  계산만 남기되 기준을 swipe → `.hero__copy` 실측으로 교체.
- `pausedRef`/`userPaused`/`hoverPaused`/`syncPaused`/`togglePause`/`nextSlide`/
  `totalLabel` 전부 미사용이 되어 삭제. 일시정지 UI가 없으므로 rAF 루프에서
  pause 분기도 제거.
- `--hero-swipe-scale/-w/-reserve`, `--hero-thumb-*`, `--hero-swipe-meta-*`,
  `--hero-frame-gap/-swipe-gap` 토큰과 1600/1280 스케일 램프 미디어쿼리 삭제.

### 히어로: 이미지 풀블리드
프레임이 좌측 인셋(카피 시작 +200px)이었던 걸 `inset: 0`으로 바꿔 브라우저 폭
100%. **부작용 하나**: 카피가 검은 여백이 아니라 사진 위에 앉게 되어 `.hero__desc`
(#999)가 밝은 슬라이드에서 안 읽혔다 → 베일에 하단 스크림 레이어 추가
(`180deg, 투명 45% → 0.82 100%`) + desc `#999` → `#ccc`. 모바일(≤767)은 베일이
`none`이라 영향 없음(포트레이트 아트가 자체 톤을 갖고 있음).

### 히어로: 등장 속도 2배 + 나눔명조
- `REVEAL_PACE = 2` 상수로 TSX의 baseDelay/step 전부를 스케일.
- CSS 트랜지션도 2배(char 0.72/0.82 → 1.44/1.64, line 0.95/1.1/1.05 → 1.9/2.2/2.1).
  **`.hero` 스코프 필수** — `.char-reveal`/`.line-reveal`은 HomeSections와 공유라
  전역으로 늦추면 홈 전체가 느려진다.
- `.hero__copy`에 `font-family: var(--font-serif)`(나눔명조). label/desc는 상속.

### 협력사 무한 띠 (`PartnerMarquee`)
`SiteLayout`에서 `isHome`일 때 Footer **바로 위**에 마운트.
- 에셋: `scripts/generate-partner-mono.py` 신규. 원본 19개는 전부 **흰 판때기 위
  어두운 아트**라 어두운 띠에 그대로 얹으면 안 보인다. 배경 키아웃(기존 알파 있으면
  그대로, 없으면 테두리 최빈색을 톨러런스 32로 제거) → 그레이스케일 → 어두운 아트는
  반전 → **레벨 정규화**(원본 반전만으로는 평균 휘도가 125~242로 흩어져 제각각으로
  보였다 → 120~255로 스트레치, 결과 157~243) → 컨텐츠 크롭 + 패딩. 원본은 그대로
  두어 법인소개 협력사 그리드는 컬러 유지.
- 루프: 리스트를 2벌 렌더 후 `translate3d(-50%)`. **간격은 `gap`이 아니라 아이템
  `margin-right`** — `gap`이면 트랙 폭이 `2×내용 + (2n−1)×gap`이라 -50%가 copy2
  시작점과 어긋나 매 사이클 점프한다. 실측 drift 0px 확인.
- `prefers-reduced-motion`에서 애니메이션 정지. 중복 사본은 `aria-hidden`.
- 로고 `alt=""` — 데이터에 회사명이 없어 지어내면 접근성 트리에 허위 기관명이
  들어간다. 대신 섹션에 `aria-label="협력사"`.

### 메뉴명 3건
`업무사례 → 분야별 사례`, `정비사업 → 정비사업 · 실적`, `공익사업 → 공익사업 · 실적`.
`nav.ts` 라벨 + 각 페이지 히어로 타이틀(`*_PAGE.title`) + `searchIndex` 문서 제목까지
같이 변경. **페이지 내부 섹션 제목(`RENEWAL_OVERVIEW.title` 등)과 로컬 탭 라벨은 유지** —
그건 메뉴가 아니라 섹션 이름이고, 각 페이지엔 이미 별도의 `정비사업실적` 탭이 있다.
홈 `content.ts`의 업무영역 카드/실적 행 제목도 유지(메뉴가 아니라 콘텐츠).

### 검증
1440/1024/390에서 이미지 풀블리드(1440×900, 1024×800, 390×844), 화살표 108/84/56
세로중앙·좌우끝, swipe 잔존 요소 0, copy 폰트 나눔명조, char 트랜지션 1.44/1.64s 확인.
화살표 클릭 01→02→03→02→01→05(랩) 정상. 서브페이지 3종 히어로 타이틀/부모칩 일치.
띠: 로고 38개(19×2) 로드 실패 0, 배경 #17191d, 루프 drift 0px, reduced-motion 정지.
classic.html 엔트리도 히어로/화살표/띠 렌더 + 콘솔 에러 0. lint/tsc/build 통과.

**환경 주의**: 이 샌드박스는 CDN 폰트(Pretendard/Nanum Myeongjo)가 egress 차단이라
스크린샷의 세리프는 폴백(Georgia)이다. `font-family` 계산값은 정상 확인했고,
프로덕션에서는 Google Fonts로 로드된다.

## 2026-08-05 — GNB 풀메뉴: 배경이미지 제거 + 블랙 리퀴드글라스

### 배경이미지 제거
서브메뉴 hover마다 바뀌던 per-menu 사진(`.gnb__visual` / `.gnb__visual-img`)과
그걸 감싸던 24px 프레임을 전부 제거. TSX에서 `visualSrc`/`visualKey` 상태와
`swapVisual()`이 같이 죽어서 `highlightSub(subId)`(하이라이트만)로 축소.
`nav.ts`의 `visual` 필드와 `GNB_SUB_VISUAL_*` 상수는 **유지** — 각 페이지 히어로
이미지의 별칭이라 페이지들이 쓰고 있다. GNB만 더 이상 참조하지 않는다.

### 블랙 리퀴드글라스
`--gnb-panel-bg: rgba(12,13,16,.72)` + `--gnb-panel-blur: blur(24px) saturate(1.5)`
토큰을 `.gnb`에 두고 **바와 패널이 같이** 쓴다.
- `.gnb--open.gnb--desktop`이 흰 배경(`#fff`)이었던 걸 같은 글래스로 교체. 안 그러면
  어두운 드롭다운 위에 흰 띠가 따로 붙어 보인다. 토큰(`--gnb-fg` 등)도 라이트로 반전.
- `.gnb__fullmenu` / `-inner`도 `#f7f7fb` → 글래스, `.gnb__columns`는 `#fff` → 투명.
- 사진과 프레임이 사라졌으니 높이도 축소: `is-open` max-height 400→340,
  inner min-height 362→300 (가장 긴 컬럼 6개 × 44px + 패딩).
- `@supports not (backdrop-filter)` 폴백 추가.
- **HARD RULE 확인**: 빌드 산출물에 `-webkit-backdrop-filter`와 unprefixed가 **둘 다**
  나오는 것 확인(`var()` 값이어도 lightningcss가 정상 처리).

### 서브메뉴 / 인디케이터 색
- `.gnb__sublink` `#767676`→`rgba(255,255,255,.7)`, hover/active `#111`→`#fff` +
  `rgba(255,255,255,.1)` 워시.
- **0.7은 취향이 아니라 대비 하한**: 패널이 반투명이라 라이트 콘텐츠 위에 겹치는
  구간에서 배경이 실측 rgb(80,80,83)이 된다. 흰색 0.62면 4.24:1로 AA 미달,
  0.7이면 4.91:1로 통과. 글래스를 더 어둡게 만들지 않고 해결.
- 인디케이터: `.gnb--open.gnb--desktop .gnb__indicator`를 `rgba(255,255,255,.16)`로
  **맨 뒤에** 추가. `[data-gnb-theme='light']` 규칙들과 명시도가 같아서, 헤더 아래
  섹션이 밝을 때 어두운 칩이 어두운 바에 얹혀 안 보이는 걸 소스 순서로 이긴다.
- 워드마크: `solid`에 `menuOpen`이 포함돼 있어 열면 다크 마크가 나왔다. 열린 상태를
  먼저 판정해 **라이트 마크**를 쓰도록 변경(나머지 `solid`= 스크롤+호버 흰 바는 그대로).

### 검증
1440에서 이미지 노드 0개, 바/패널 모두 `rgba(12,13,16,.72)`+동일 blur, 컬럼 투명,
서브링크 rest `rgba(255,255,255,.7)` / active 흰색, 인디케이터 `rgba(255,255,255,.16)`,
5개 대메뉴 전부 항목 잘림 0. 라이트 섹션 위로 스크롤 후 열어도 바가 다크 유지 +
인디케이터 라이트 + 워드마크 라이트 확인. 모바일 드로어는 흰색 그대로(미변경).
lint/tsc/build 통과.

### 기존 결함(이번 변경과 무관, 미수정)
`기타업무`의 `상속 · 이혼 · 가사`와 `활동 · 보도`의 `컬럼미디어`가 겹쳐 보인다
("가사컬럼미디어"). 컬럼이 상단 nav 항목 좌표/너비에 스냅되는데 서브라벨이
`white-space: nowrap`이라 좁은 컬럼을 넘치는 구조적 문제. **변경 전 빌드로
스크린샷 찍어 동일하게 재현되는 것 확인** — 이번 리컬러가 만든 게 아니다.
고치려면 컬럼 폭 산정 방식을 바꿔야 해서(레이아웃 결정) 별도 처리 필요.
