...
### 미결 — 슬라이드 04/05 오브젝트 가시성
390×844 풀블리드는 1600×900 원본의 가로 26%만 남아, 05는 건물 벽면만 보여 법원으로
인지되지 않고 04는 가로등 아랫부분이 카피에 가린다. 세 안을 실제 화면으로 렌더해
사용자에게 제시:
- 이미지 68% + 슬라이드별 앵커 유지 → 04 가로등+비둘기 전체, 05 타워+현관
  아케이드까지 인지됨. 하단 32%는 카피용 어두운 밴드. 태블릿(58%)과도 일관.
- 풀블리드 유지(현행)
- contain + 블러 백드롭 → 오브젝트는 다 보이나 레터박스 스트립이라 히어로로 약함
사용자 선택 대기 중. 68% 안을 권장으로 제시했다.
