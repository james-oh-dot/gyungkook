...
## 2026-07-20 — 공익사업 "절차 · 유의할 점" 흐름 모바일 재설계

> Branch: `claude/o-boinida-98drdm` (PR #73 머지 후 최신 main에서 재시작)
> 파일: `src/pages/PublicProject.css`, `src/pages/PublicProjectPage.tsx`.

### 진단 (모바일 UX)
가로 넘침은 없었으나(steps/outcomes 1열), 손실보상절차 흐름이 **분리된 카드 스택**이라:
1. 절차의 **순서·방향**(1→2→3→분기→4→5→6)이 전달 안 됨.
2. **분기 로직 소실** — 협의 성립(→종결) vs 불성립(→4~6 진행) 관계가 안 드러남.
3. 단계 간 연결선 없음 → 리스트처럼 보임.
Figma 원본은 커넥터가 있는 **세로 플로우차트**라, 기존 3열 그리드는 데스크톱에서도 흐름을
잘 못 보여줌.

### 재설계 (세로 타임라인 + 명시적 분기 — 전 구간 공통)
- **타임라인**: `.pp-flow__steps`를 grid→세로 스택으로. 번호 배지를 잇는 **척추(spine)**를
  `.pp-flow-step:not(:last-child)::before`(left 13px = 배지 중심, top 14px→bottom 0)로
  그려 다음 배지까지 연결. 배지는 `z-index:1`로 선 위에. 스텝 카드 테두리 제거(깔끔한 스텝퍼).
- **분기 명시**: 스텝 3 뒤에 `↓ 협의 결과에 따라` 라벨 + 결과 박스 2개.
  성립(teal, 태그 "절차 종결"), 불성립(red, 태그 "아래 후속 절차 진행").
  이어서 `↓ 협의 불성립 시 진행` 라벨 + 후속 스텝 4~6 타임라인(불성립 분기임을 표시,
  데스크톱은 44px 들여쓰기로 정렬).
- **반응형**: 데스크톱/태블릿≥769는 결과 박스 2열 + 44px 정렬; ≤768은 결과 1열 +
  들여쓰기 해제(라벨·태그가 분기 로직을 전달). 세로 타임라인은 전 구간 동일.

### 검증 (Playwright, vite preview)
- 390 / 768 / 1440 모두: 세로 타임라인 + 척추 연결선, 분기 라벨·태그 렌더, **가로 넘침 0**.
- build(strict)·lint 클린. (두 번째 흐름 [재개발], 하단 권리 카드 영향 없음.)
