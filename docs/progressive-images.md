...
- **홈 히어로** (`Hero` / `slides.ts`): `hero-01` … `hero-05`
  - Ken Burns / per-slide framing은 `.progressive-image` 래퍼에 유지
  - preview/full 레이어가 박스를 채움 (statue brightness는 full+preview 각각)
  - 첫 슬라이드 `priority` + 전 슬라이드 `loading="eager"` + mount 시 preload
...
- 히어로 swipe thumb (`nextImage`)도 progressive 쌍으로 교체
